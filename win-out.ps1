...
1- :-> $password = Read-Host -AsSecureString # TO CREATE PASSWORD
   :-> New-LocalUser -Name "{your user name}" -Password $password -FullName "your full name" -Description "add new user"
   # add a user to a group :
   -> Add-LocalGroupMember -Group "{your group}" -Member "{your member}"
2: powershell script:
<#
.SYNOPSIS
...
$logFile = "\\server\folder\log.txt" # C:\Windows\System32\winevt\Logs
